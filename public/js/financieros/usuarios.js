// JavaScript completo actualizado para control de usuarios
let editandoId = null;

// Tabla
const tabla = $('#tablaUsuarios').DataTable({
  ajax: { url: '/api/financieros/usuarios/', dataSrc: '' },
  columns: [
...
$('#tablaUsuarios').on('click', '.btn-editar', async function () {
  ...
  try {
    const res = await fetch(`/api/financieros/usuarios/${id}`);
...
$('#tablaUsuarios').on('click', '.btn-eliminar', async function () {
  ...
  if (confirmacion.isConfirmed) {
    try {
      const res = await fetch(`/api/financieros/usuarios/${id}`, { method: 'DELETE' });
      if (res.ok) {
        tabla.ajax.reload(null, false);
...
$('#formPasswordCambio').on('submit', async function (e) {
  ...
  try {
    const res = await fetch(`/api/financieros/usuarios/password/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
